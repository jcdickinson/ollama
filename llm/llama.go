...
func chooseRunners(workDir, runnerType string) []ModelRunner {
	...
	case "linux":
		runners = []ModelRunner{}
		if acceleratorName != "none" {
			runners = []ModelRunner{ModelRunner{
				Path:        path.Join(buildPath, acceleratorName, "bin", "ollama-runner"),
				Accelerated: true,
			}}
		}
		runners = append(runners, ModelRunner{Path: path.Join(buildPath, "cpu", "bin", "ollama-runner")})
	case "windows":
